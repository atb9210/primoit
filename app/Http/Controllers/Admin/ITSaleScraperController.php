        ...
        try {
            ...
            if ($listResponse->successful()) {
                ...
                if (preg_match_all('/Mix\s*Units\s*(\d+)\s*Average\s*(?:€|&euro;)?([\d,.]+)([^(]*)\(([^)]+)\)(?:[^€]*(?:€|&euro;)([\d,.]+))?(?:[^€]*(?:€|&euro;)([\d,.]+))?/i', $bodyText, $matches, PREG_SET_ORDER)) {
                    Log::info('Found ' . count($matches) . ' lists with method 2 from /list');
                ...
                foreach ($lines as $line) {
                    ...
                    $list = [
                        ...
                        'description' => $description,
                                'price' => isset($prices[1]) ? '€' . $prices[1] : (isset($prices[0]) ? '€' . $prices[0] : ''),
                                'discounted_price' => isset($prices[2]) ? '€' . $prices[2] : (isset($prices[1]) ? '€' . $prices[1] : '')
                            ];
            ...
            if ($homepageResponse->successful()) {
                ...
                if ($listsDiv->count() > 0) {
                    ...
                    // Nuova implementazione: estrai direttamente dai span per Latest Wholesale Lists
                    $items = $listsDiv->filter('.list-card-wrapper-link');
                    
                    if ($items->count() > 0) {
                        Log::info('Found ' . $items->count() . ' items with class list-card-wrapper-link');
                        
                        $items->each(function (Crawler $item, $i) use (&$latestLists, $listsDiv) {
                            // Estrai il nome direttamente dal span
                            $name = trim($item->text());
                            
                            // Recuperiamo il testo di tutto il blocco lists
                            $listsText = $listsDiv->text();
                            
                            // Cerchiamo il contesto del nome nell'intero testo
                            $contextSize = 200; // Caratteri prima e dopo il nome
                            $namePos = strpos($listsText, $name);
                            
                            if ($namePos !== false) {
                                $start = max(0, $namePos - $contextSize);
                                $length = strlen($name) + (2 * $contextSize);
                                $parentText = substr($listsText, $start, $length);
                            } else {
                                // Fallback: usa tutto il testo
                                $parentText = $listsText;
                            }
                            
                            // Cerca Units
                            $units = '';
                            if (preg_match('/Units\s*(\d+)/i', $parentText, $unitMatches)) {
                                $units = $unitMatches[1];
                            }
                            
                            // Cerca Average Price
                            $avgPrice = '';
                            if (preg_match('/Average\s*(?:€|&euro;)?([\d,.]+)/i', $parentText, $avgMatches)) {
                                $avgPrice = '€' . $avgMatches[1];
                            }
                            
                            // Cerca Description
                            $description = '';
                            if (preg_match('/\(([^)]+)\)/i', $parentText, $descMatches)) {
                                $description = trim($descMatches[1]);
                            }
                            
                            // Cerca Price
                            $prices = [];
                            if (preg_match_all('/(?:€|&euro;)([\d,.]+)/i', $parentText, $priceMatches)) {
                                $prices = $priceMatches[1];
                            }
                    ...
                    // Se non trova elementi con quella classe, usa i metodi precedenti
                    if (empty($latestLists)) {
                        ...
                        // Altri metodi fallback come prima...
